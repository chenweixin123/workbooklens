    ...
    results = [
        {
            ...
            "locations": [_sarif_location(finding, scan.inspection.path.name)],
        ...
        "runs": [
            {
                "tool": {
                    "driver": {
                        ...
                        "informationUri": "https://github.com/workbooklens/workbooklens",
                        "rules": rules,
                    }
                },
                "automationDetails": {"id": f"workbooklens/{scan.inspection.path.name}"},
                "results": results,
            }
        ],
    }
...
def write_scan_report(scan: ScanResult, output_directory: Path) -> dict[str, Path]:
    ...
    findings_payload = {
        ...
        "risk_score": _risk_score(scan.findings),
    ...
    write_json(sarif_path, build_sarif(scan))

    severity_counts = Counter(finding.severity.value for finding in scan.findings)
    rule_counts = Counter(finding.rule_id for finding in scan.findings)
