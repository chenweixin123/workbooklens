...
_MESSAGE_PAIRS: dict[str, tuple[str, str]] = {
    ...
    "risk.formula_derived": ("Formula-derived", "公式推导"),
    "risk.semantic_review": ("Semantic review", "需确认语义"),
    "recalc_provider.none": ("Not available", "不可用"),
    "recalc_provider.excel": ("Microsoft Excel", "Microsoft Excel"),
    "recalc_provider.libreoffice": ("LibreOffice", "LibreOffice"),
    "validation_status.not_run": ("Not run", "未运行"),
    "validation_status.passed": ("Passed", "已通过"),
    "validation_status.degraded": ("Degraded safely", "已安全降级"),
    "validation_status.failed": ("Failed and rolled back", "失败并已回滚"),
    ...
    "patch_kind.normalize_text": ("Normalize value", "规范化数值"),
    ...
    "web.privacy_body": (
        "Files remain in a process-owned local temporary directory and are removed during normal shutdown. Scanning and direct OOXML repair do not execute formulas, macros, external links, or embedded objects. Only explicitly authorized trusted-file conversion or formula validation opens temporary copies in Excel or LibreOffice.",
        "文件保存在当前进程专用的本地临时目录中，并在正常退出时删除。扫描和直接 OOXML 修复不会执行公式、宏、外部链接或嵌入对象。只有明确授权的可信文件转换或公式验证才会用 Excel 或 LibreOffice 打开临时副本。",
    ),
    ...
    "web.profile_label": (
        "Workbook Profile (optional .yml or .yaml)",
        "工作簿 Profile（可选 .yml 或 .yaml）",
    ),
    "web.profile_hint": (
        "Adds user-owned column semantics after strict local validation. The profile is limited to 1 MB, stored under a server-owned name, and never treated as a local path.",
        "通过严格的本地校验后添加用户定义的列语义。Profile 上限为 1 MB，以服务器固定名称保存，绝不会被当作本地路径使用。",
    ),
    ...
    "web.results_select_all": ("Select safe + formulas", "全选安全项与公式项"),
    ...
    "web.results_semantic_consent": (
        "I reviewed and accept the semantic value changes",
        "我已检查并接受这些业务语义值更改",
    ),
    "web.results_recalculation_consent": (
        "I trust this workbook for isolated local recalculation",
        "我信任此工作簿并允许隔离的本地重算",
    ),
    "web.recalculation_consent_body": (
        "Excel or LibreOffice will open temporary copies and may evaluate WEBSERVICE, data connections, or other workbook-defined behavior. Leave this unchecked for untrusted files; lossless repairs still run and formula candidates are skipped.",
        "Excel 或 LibreOffice 将打开临时副本，并可能计算 WEBSERVICE、数据连接或其他工作簿内置行为。不可信文件请勿勾选；无损修复仍会执行，公式候选将被跳过。",
    ),
    "web.semantic_consent_body": (
        "Semantic repairs can change interpreted business values. They are applied only when explicitly selected and authorized here.",
        "语义修复可能改变系统对业务值的解释；只有明确选择并在此授权后才会应用。",
    ),
    "web.results_auto_repair": ("One-click safe repair", "一键安全修复"),
    "web.results_auto_repair_help": (
        "Applies lossless normalization and uniquely derived formulas. Formula changes require the trusted-workbook checkbox and must pass isolated local recalculation; otherwise they are skipped. Semantic and layout repairs remain unselected.",
        "应用无损规范化和唯一推导公式。公式更改必须勾选可信工作簿并通过隔离的本地重算，否则会被跳过。语义与布局修复不会被自动选择。",
    ),
    "web.patch_group_safe": ("Lossless normalization and safe repairs", "无损规范化与安全修复"),
    "web.patch_group_safe_help": (
        "Deterministic safe changes that preserve the intended value and format.",
        "保持预期数值和格式的确定性安全更改。",
    ),
    "web.patch_group_formula_derived": ("Formula-derived repairs", "公式推导修复"),
    "web.patch_group_formula_derived_help": (
        "Unique formula candidates supported by independent evidence and applied only after trusted, isolated local recalculation succeeds.",
        "由相互独立证据支持的唯一公式候选，仅在用户信任并且隔离的本地重算成功后应用。",
    ),
    "web.patch_group_semantic_review": ("Semantic confirmation required", "语义需确认"),
    "web.patch_group_semantic_review_help": (
        "Business-value interpretations that require explicit human confirmation.",
        "需要人工明确确认的业务值解释。",
    ),
    "web.patch_group_layout_review": ("Layout confirmation required", "布局需确认"),
    "web.patch_group_layout_review_help": (
        "Widths, heights, wrapping, borders, or saved views that require visual review.",
        "需要目视检查的列宽、行高、换行、边框或保存视图更改。",
    ),
    ...
    "web.patch_derivation_evidence": ("Derivation evidence", "推导证据"),
    "web.patch_derivation_strategy": ("Strategy", "推导策略"),
    "web.patch_candidate_count": ("Candidate count", "候选数量"),
    "web.patch_evidence_sources": ("Evidence sources and peer support", "证据来源与同行支持"),
    "web.patch_invariants": ("Preserved constraints", "保持约束"),
    "web.patch_requires_recalculation": ("Requires recalculation", "是否需要重算"),
    ...
    "web.applied_recalculation_title": ("Validation and rollback", "验证与回滚"),
    "web.applied_provider": ("Recalculation provider", "重算提供者"),
    "web.applied_formula_before": ("Formula errors before", "修复前公式错误"),
    "web.applied_formula_after": ("Formula errors after", "修复后公式错误"),
    "web.applied_validation_status": ("Validation status", "验证状态"),
    "web.applied_rollback": ("Rollback performed", "是否执行回滚"),
    "web.applied_downgraded": ("Downgraded repairs", "已降级修复"),
    "web.applied_skipped": ("Skipped repairs", "已跳过修复"),
    "web.applied_locations_title": ("Modified locations", "修改位置"),
    "web.applied_no_locations": ("No workbook locations were changed.", "未更改任何工作簿位置。"),
    "web.applied_none": ("None", "无"),
    "web.error_title": (
    ...
    "web.error_repair_status_title": ("Repair validation result", "修复验证结果"),
    "web.error_failure_report_download": (
        "Download bilingual failure report",
        "下载中英文失败报告",
    ),
    ...
    "web.applied_intro": (
        "The output copy completed WorkbookLens validation without changing the source file.",
        "输出副本已完成 WorkbookLens 验证，源文件保持不变。",
    ),
    ...
    "cli.plan_summary": (
        "{patches} patches ({safe} safe, {formula} formula-derived, {semantic} semantic review, {layout} layout review) → {output}",
        "{patches} 项修复（{safe} 项安全修复，{formula} 项公式推导，{semantic} 项需确认语义，{layout} 项需检查布局）→ {output}",
    ),
    ...
    "cli.apply_validation_summary": (
        "Recalculation: {provider}; formula errors {before} → {after}; validation: {status}; downgraded: {downgraded}; skipped: {skipped}",
        "重算：{provider}；公式错误 {before} → {after}；验证：{status}；降级：{downgraded}；跳过：{skipped}",
    ),
    ...
    "report.limitations_body": (
        "Core WorkbookLens analysis does not calculate formulas and does not claim complete Excel compatibility. With explicit trusted-workbook authorization, uniquely derived formula repairs may be validated by an installed Excel or LibreOffice process using isolated temporary copies; that application may process workbook-defined behavior. Without authorization or a provider, formula candidates are not applied. Shared, array, data-table, dynamic-array, and ambiguous formulas are not automatically repaired.",
        "WorkbookLens 核心分析不计算公式，也不声称完全兼容 Excel。只有在用户明确授权可信工作簿后，唯一推导的公式修复才可由本机 Excel 或 LibreOffice 使用隔离临时副本进行验证；该应用可能处理工作簿内置行为。未授权或没有可用提供者时，不会应用公式候选。共享公式、数组公式、数据表公式、动态数组公式和含义不明确的公式不会自动修复。",
    ),
    "report.generated": (
