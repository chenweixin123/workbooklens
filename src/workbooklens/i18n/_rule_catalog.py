...
BUILTIN_RULE_TITLES: dict[str, tuple[str, str]] = {
    ...
    "WL057_CROSS_SHEET_ORPHAN_IDENTIFIER": (
        "Identifier absent from inferred parent table",
        "标识符未出现在推断父表中",
    ),
    "WL058_LOSSLESS_VALUE_NORMALIZATION": (
        "Lossless value normalization candidate",
        "无损值规范化候选",
    ),
}
...
ZH_CANONICAL_TEXT: dict[str, str] = {
    "A literal cell has a unique normalization candidate supported by its column role and peers": "一个常量单元格具有由列角色和同行值共同支持的唯一规范化候选。",
    "A literal value can be normalized without inventing a business value; automatic application is allowed only when parsing, role, peer storage, number format, and target safety all have one supported outcome.": "一个常量值可以在不臆造业务值的前提下规范化；只有解析、角色、同行存储、数字格式和目标安全性均得到唯一支持时才允许自动应用。",
    "Literal values use native storage while preserving their reviewed semantic role and display format.": "常量值应使用原生存储，同时保留经复核的语义角色和显示格式。",
    "Apply the lossless normalization and its number-format companion as one atomic repair.": "将无损规范化及其数字格式配套补丁作为一个原子修复应用。",
    "Review and explicitly authorize the semantic normalization candidate.": "请复核并明确授权该语义规范化候选。",
    "Review the reported candidate; WorkbookLens did not apply it because one or more safety constraints were not proven.": "请复核报告中的候选；由于一个或多个安全约束尚未得到证明，WorkbookLens 未应用该候选。",
    "Copy only the unique peer-consensus number format; preserve all other style fields.": "仅复制唯一的同行共识数字格式，并保留其他所有样式字段。",
    "Remove a uniquely identified trailing-whitespace suffix while preserving style.": "在保留样式的同时移除唯一确定的尾随空白后缀。",
    "Store a uniquely parsed literal as its native OOXML numeric value.": "将唯一解析的常量保存为原生 OOXML 数值。",
    "A value in a repeated identifier column is absent from a more unique cross-sheet column with the same header and strong value overlap.": "重复标识符列中的一个值未出现在另一张工作表上表头相同、唯一性更高且值高度重合的列中。",
    "Identifier value is absent from the inferred parent column": "标识符值未出现在推断父列中",
    "Foreign-key values exist in the inferred parent identifier column.": "外键值应存在于推断的父标识符列中。",
    "Review the child value and the inferred parent table; WorkbookLens does not invent or replace identifiers.": "请复核子表值及推断父表；WorkbookLens 不会臆造或替换标识符。",
    ...
    "Convert the isolated formula text to the unique translated peer formula.": "将孤立的文本公式转换为参照公式唯一平移得到的真实公式。",
    ...
    "Extend the aggregate to the unique complete inferred table body.": "将聚合范围扩展到唯一确定的完整推断表格主体。",
    "Synchronize every conditional-aggregate range to the unique complete inferred table body.": "将条件聚合中的所有范围同步扩展到唯一确定的完整推断表格主体。",
    ...
    "A summary-labelled conditional aggregate uses aligned criteria and sum ranges that all stop before or start after the same contiguous source-table rows.": "一个带汇总标签的条件聚合使用了对齐的条件范围与求和范围，而这些范围都在同一批连续源表行之前结束或之后开始。",
    ...
    "Conditional aggregate ranges synchronously omit contiguous source-table rows": "条件聚合的多个范围同步遗漏了连续的源表行",
    "COUNTIF denominator is proven zero from literal source values": "根据源表常量值可证明 COUNTIF 分母为零",
    ...
    "Confirm that the criteria and sum ranges should share the reviewed source-table boundary.": "请确认条件范围与求和范围是否应共享经复核的源表边界。",
    ...
    "Review every conditional-aggregate range against the complete source table body; formulas are not changed automatically.": "请依据完整源表主体复核条件聚合中的每个范围；公式不会自动更改。",
    ...
    "Review the COUNTIF condition and intended KPI denominator; WorkbookLens does not infer a replacement formula.": "请复核 COUNTIF 条件和预期 KPI 分母；WorkbookLens 不会推断替换公式。",
    ...
    "The formula denominator is nonzero for the current literal inputs.": "对于当前常量输入，公式分母应为非零值。",
    ...
    "Every known numeric literal in the COUNTIF range fails the comparison, so the denominator is statically zero.": "COUNTIF 范围内所有已知数值常量都不满足比较条件，因此可静态证明分母为零。",
